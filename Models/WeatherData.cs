...
namespace WeatherUndergroundAPI.Models
{
    public class WeatherData
    {
        ...
        public JObject CreateObjectByZip(string zip)
        {
            HttpWebRequest request = WebRequest.CreateHttp(URLheader + APIkey + param + zip + extension);
        ...
        public JObject CreateObjectByCityState(string city, string state)
        {
            string citystate = state + "/" + city;
            HttpWebRequest request = WebRequest.CreateHttp(URLheader + APIkey + param + citystate + extension);
            request.UserAgent = @"User-Agent: Mozilla/5.0 (Windows NT 10.0; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/53.0.2785.116 Safari/537.36";
            HttpWebResponse response = (HttpWebResponse)request.GetResponse();
            StreamReader rd = new StreamReader(response.GetResponseStream());
            string ApiText = rd.ReadToEnd();
